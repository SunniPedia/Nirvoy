...
public class AllphonenumberActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.allphonenumber);
	...
	private void initializeLogic() {
		...
		try {
			InputStream inputstream1 = getAssets().open(getIntent().getStringExtra("heading"));
			map = new Gson().fromJson(RizwanUtil.copyFromInputStream(inputstream1), new TypeToken<ArrayList<HashMap<String, Object>>>() {
			}.getType());
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.number, null);
			}
			...
			textview2.setTypeface(Typeface.createFromAsset(getAssets(), "fonts/myfont.ttf"), android.graphics.Typeface.NORMAL);
			textview1.setText(map.get((int) _position).get("thana").toString());
			textview2.setText(map.get((int) _position).get("number").toString());
			linear1.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View _view) {
					in.setAction(Intent.ACTION_CALL);
					in.setData(Uri.parse("tel:".concat(map.get((int) _position).get("number").toString())));
					startActivity(in);
				}
			});
